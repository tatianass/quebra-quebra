...
### Custo de uma Ambulância

```{r custo_de_uma_ambulancia}
articles <- 125
valueBox(articles, icon = "fa-car", color="primary")
```

### Meses de bolsa CAPES de doutorado 

```{r bolsa_capes_doutorado}
articles <- 123
valueBox(articles, icon = "fa-pencil", color="#2a6592")
```

### Meses de tratamento do Câncer

```{r tratamento_cancer}
articles <- 12
valueBox(articles, icon = "fa-medkit", color="primary")
```

### Meses de merenda escolar

```{r meses_merenda_escolar}
articles <- 56
valueBox(articles, icon = "fa-cutlery", color="#2a6592")
```

### Domésticas contribuindo para o INSS

```{r domestica_aposentadoria}
articles <- 543
valueBox(articles, icon = "fa-home", color="primary", href="https://www.domesticalegal.com.br/utilidades/tabela-de-inss/")
```

### Kilomêtros de rodovias asfaltadas

```{r asfaltar_rodovias}
comments <- 21515
valueBox(comments, icon = "fa-road", color="#2a6592")
```
