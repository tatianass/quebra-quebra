---
title: "Comparação Folha Senado com Setor Privado"
author: "Leandro"
date: "26/07/2017"
...
data_filename = "~/Documents/quebra-quebra/senado_federal/dados/dados_senado.csv"
...
```
## Funcionalismo Público vs Privado
...
```{r aposentados, cache=F, echo=FALSE}
...
#ggplot(data=df, aes(x=categorias, y=valores,fill=categorias)) +
...
[Matéria do Estadão sobre aposentadoria média de parlamentares](http://economia.estadao.com.br/noticias/geral,aposentadoria-media-de-parlamentar-e-7-5-vezes-superior-a-media-do-inss,10000080033)


```{r set-options, echo=FALSE, cache=FALSE}
options(RCHART_WIDTH = 950, RCHART_HEIGHT = 400)
```


```{r, echo=FALSE, cache=F, results='asis', comment=NA}
p1 = mPlot(x = 'categorias', y = list('valores'), data = df, type = 'Bar')
...
```{r comparacao, echo=FALSE, include=FALSE}
...
#new_data[,3] = sapply(new_data[,3],as.numeric)
#new_data[,3] = sapply(new_data[,3],round,0)
...
```{r, echo=FALSE, cache=F, results='asis', comment=NA}
n1 = nPlot(value ~ Especialidade,group="variable", data=new_data, type="multiBarChart")
n1$chart(showControls = F)
n1$print('chart2',include_assets = T)
```
...
```{r, echo=FALSE, cache=F, results='asis', comment=NA}
y1 = mPlot(x = 'Especialidade', y = list('diferença'), data = diferenca, type = 'Bar')
y1$set(hideHover = "auto")
y1$print('chart3',include_assets = T)
```

Fontes:

[Matéria do Estadão comparando os salários do funcionalismo público e provado](http://economia.estadao.com.br/noticias/geral,em-13-anos-salario-do-servico-publico-subiu-tres-vezes-mais-que-o-privado,10000079369)
...
[Sobre o Piso Salarial de Profissionais de Enfermagem (consideramos R$ 3.940 que corresponde a 6-8 anos de experiência)](http://seesp.com.br/pisos/)

[Salários de Profissionais de Enfermagem em 2016](http://www.guiadacarreira.com.br/salarios/piso-salarial-enfermeiro/ )
