...
valueBox(value, caption="Ambulâncias novas", icon = "fa-ambulance", color=cor_do_site, href="http://www.otempo.com.br/cidades/dezenove-ambul%C3%A2ncias-do-samu-est%C3%A3o-paradas-em-galp%C3%A3o-1.1436254")
```
...
valueBox(value, caption="Alunos de doutorado com Bolsa CAPES por 4 anos", icon = "fa-pencil", color=cor_do_site, href="http://www.capes.gov.br/bolsas/valores-de-bolsas")
```
...
valueBox(value, caption="Atendimentos à pacientes com Câncer", icon = "fa-medkit", color=cor_do_site, href="http://www2.inca.gov.br/wps/wcm/connect/agencianoticias/site/home/noticias/2016/ministro-da-saude-anuncia-prioridades-para-a-oncologia")
```
...
valueBox(value, caption="Crianças beneficiadas com merenda escolar", icon = "fa-cutlery", color=cor_do_site, href="http://agenciabrasil.ebc.com.br/politica/noticia/2017-02/governo-anuncia-reajustes-para-merenda-escolar")
```
...
valueBox(value, caption="Domésticas contribuindo para o INSS", icon = "fa-home", color=cor_do_site, href="https://www.domesticalegal.com.br/utilidades/tabela-de-inss/")
```
...
valueBox(value, caption="Cestas básicas", icon = "fa-shopping-cart", color=cor_do_site, href="https://www.dieese.org.br/cesta/")
```
...
### Teto Constitucional
```{r prepara_dados_teto_constitucional, echo=FALSE}
cor_do_site = "#2c3e50"
teto_constitucional <- 37476.93
deputados_acima_teto <- todos_deputados[todos_deputados$remuneracao_total > teto_constitucional, ]
...
servidores_acima_teto_recente <- todos_servidores_recente[todos_servidores_recente$remuneracao_total > teto_constitucional, ]
```

* **Média salarial por cargo e vínculo**


```{r plot_media_salarial_por_cargo, echo=FALSE}
title <- "Média salarial por cargo"
dados_grafico <- aggregate(remuneracao_total~cargo+vinculo, mean, data=data)

g <- ggplot(dados_grafico, aes(cargo, remuneracao_total, fill=vinculo)) +
    geom_bar(position = "dodge", stat = "identity") + coord_flip() +
    labs(y="Remuneração Total (R$)", x="Cargo", title=title) +
    theme_light()
...
g <- ggplot(dados_grafico, aes(x=nome, remuneracao_total)) +
...
title <- paste("Servidores que mais Ultrapassaram o Teto entre", ano_min, "e", ano_max)
dados_grafico <- servidores_acima_teto[,c("nome","remuneracao_total", "cargo")]
dados_grafico$remuneracao_total <- 1
dados_grafico <- aggregate(remuneracao_total~nome+cargo, sum, data=dados_grafico)
dados_grafico <- dados_grafico[order(dados_grafico$remuneracao_total, decreasing=TRUE)[1:10],]

g <- ggplot(dados_grafico, aes(x=nome, remuneracao_total, fill=cargo)) +
  geom_bar(stat = "identity") + coord_flip() +
  labs(y="Quantidade de vezes", x="", title=title) +
  theme_light()
ggplotly(g)
```
